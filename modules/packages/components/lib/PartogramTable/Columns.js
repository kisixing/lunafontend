...
exports.default = function(data) {
  return new ColumnTool(data)
    ...
    .pushColumnDataMap()
    .pushTool()
    .result.map(function(_) {
      return __assign(__assign({}, _), { align: 'center' });
    });
};
...
function pushColumnDataMap() {
  ...
  this.result = __spreadArrays(
    this.result,
    columnData_1.default.map(function(_a) {
      var title = _a.title,
        key = _a.key,
        dataset = _a.dataset,
        type = _a.type,
        width = _a.width,
        others = __rest(_a, ['title', 'key', 'dataset', 'type', 'width']);
      dataset = dataset;
      return __assign(
        {
          title: title,
  ...
  this.result = __spreadArrays(this.result, [
    {
      dataIndex: Math.random().toString(),
      width: '100px',
      title: react_1.default.createElement(button_1.default, {
        size: 'small',
        icon: 'plus',
        onClick: function() {
          _this.onCommit();
      ...
      render: function(a, b, rowIndex) {
        return react_1.default.createElement(
          ...
          react_1.default.createElement(button_1.default, {
            ...
            icon: 'check',
            onClick: function() {
              var outerId = b.outerId,
                visitTime = b.visitTime,
                pregnancy = b.pregnancy,
                doctor = b.doctor,
                other = __rest(b, ['outerId', 'visitTime', 'pregnancy', 'doctor']);
              _this.onCommit({
