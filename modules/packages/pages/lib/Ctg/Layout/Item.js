...
var WorkbenchItem = function (props) {
    ...
    return (react_1.default.createElement(antd_1.Col, { span: itemSpan, ref: ref, onClick: function () { return onSelect(unitId); }, style: { transition: 'background .6s', padding: outPadding, height: itemHeight, background: bordered ? 'black' : "var(--theme-" + 'light' + "-color)", position: 'relative' } },
        react_1.default.createElement(index_1.default, { isFullscreen: isFullscreen, themeColor: themeColor, startTime: startTime, bedname: bedname, status: status, data: data, onDoubleClick: fullScreenCb, loading: loading, onClose: onClose && (function () { return onClose(itemData); }), unitId: unitId, name: pregnancy.name, age: pregnancy.age, bedNO: pregnancy.bedNO, GP: pregnancy.GP, gestationalWeek: pregnancy.gestationalWeek, onSelect: null, RenderMaskIn: RenderMaskIn }, props.children)));
};
