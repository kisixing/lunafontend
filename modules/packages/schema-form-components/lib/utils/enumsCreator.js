...
var generator = function (Wrap, Item) {
    return function (dataset, size) {
        ...
        var Cop = function (props) {
            ...
            var target;
            return readOnly ? ((target = dataset.find(function (_) {
                _.value === value;
            })) ? (target.label) : null) : (react_1.default.createElement(Wrap, { size: size, value: value, onChange: onChange }, dataset.map(function (item) {
                return (react_1.default.createElement(Item, { value: item.value, key: item.value }, item.label));
            })));
        };
        return Cop;
    };
