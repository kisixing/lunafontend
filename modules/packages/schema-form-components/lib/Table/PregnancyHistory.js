...
var columnData = [
  ...
  return __assign(__assign({}, d), {
    children: d.children.map(function(dd) {
      return __assign(__assign({}, dd), { onCell: onCell(dd) });
    }),
  });
});
var Column = antd_1.Table.Column,
  ColumnGroup = antd_1.Table.ColumnGroup;
...
function EditableTable(props) {
  ...
  data = data.map(function(d, index) {
    return __assign(__assign({}, d), { 日期: moment_1.default(d.日期), 孕次: index + 1 });
  });
  return react_1.default.createElement(
    ...
    react_1.default.createElement(
        ...
        components: {
          body: {
            cell: function(p) {
              return react_1.default.createElement(
                EditableCell,
                __assign({}, p, { readOnly: props.readOnly })
              );
            },
          },
        },
      ...
      columnData.map(function(d, index) {
        if (!d.children) {
          return react_1.default.createElement(
            Column,
            __assign({ align: 'center' }, d, { key: index + Math.random().toString() })
          );
        }
        return react_1.default.createElement(
          ColumnGroup,
          { title: d.title, key: index + Math.random().toString() },
          d.children.map(function(dd, dd_index) {
            return react_1.default.createElement(
              Column,
              __assign({ align: 'center' }, dd, { key: dd_index + Math.random().toString() })
            );
          })
        );
      })
  ...
  antd_2.connect({})(function(props) {
    ...
    var EditableFormTable = antd_1.Form.create({
      onValuesChange: function(props, keyValue) {
        var _a = Object.entries(keyValue)[0],
          keyRowIndex = _a[0],
          cellValue = _a[1];
        var _b = keyRowIndex.split(':'),
          key = _b[0],
          rowIndex = _b[1];
        onChange(
          value.map(function(v, index) {
            ...
            if (index === parseInt(rowIndex)) {
              return __assign(
                __assign({}, v),
                ((_a = {}),
                (_a[key] = typeof cellValue === 'boolean' ? cellValue : cellValue.format()),
                _a)
              );
            }
            return v;
    ...
    return react_1.default.createElement(
      'div',
      null,
      react_1.default.createElement(EditableFormTable, {
        value: value,
        onChange: onChange,
        readOnly: readOnly,
      })
