...
export const useData = (setDevice:any,url = defaultUrl): Promise<Map<any,any>> => {
                    ...
                    for (var i in devlist) {
                        ...
                        for(let bi in devdata.beds){
                            ...
                            if(!datacache.has(cachebi)){
                                datacache.set(cachebi,{'fhr':[],'toco':[],'fm':[],'index':0,'length':0,'start':-1,'last':0,'past':0,'timestamp':0,'docid':'','starttime':''});
                           ...
                           for(let i = datacache.get(cachbi).start;i>datacache.get(cachbi).past;i--){
                               if(!tmpcache.fhr[0][i]){
                                   ...
                                   if(curstamp - tmpcache.timestamp >interval){
                                       ...
                                       //反向取值
                                       send('{"name":"get_data_ctg","data":{"start_index":'+startpoint+',"end_index":'+endpoint+',"device_no":'+id+',"bed_no":'+bi+'}}');
                        ...
                        for(let il = tmpcache.last;il<tmpcache.index;il++){
                            if(!tmpcache.fhr[0][il]){
                                ...
                                if(flag >0){
                                    ...
                                    if(curstamp - tmpcache.timestamp >interval){
                                        send('{"name":"get_data_ctg","data":{"start_index":'+sflag+',"end_index":'+eflag+',"device_no":'+id+',"bed_no":'+bi+'}}');
                                        tmpcache.timestamp = new Date().getTime();
                                        break;
                                    }
                                }else{
    ...
    function convertdocid(id: any,doc_id:string){
        ...
        if(vt.length>2){
            datacache.get(id).starttime = '20'+vt[2].substring(0,2)+'-'+vt[2].substring(2,4)+'-'+vt[2].substring(4,6)+' '+vt[2].substring(6,8)+':'+vt[2].substring(8,10)+':'+vt[2].substring(10,12);
        }
        }
    }
