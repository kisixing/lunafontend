...
class SiderMenuC extends Component<{ dataSource: { id?: number, note?: string, pregnancy: { name: string } }[], setItem: any, selected: any }, any> {
        ...
        {dataSource.map((item) => {
          return (
            <Menu.Item key={item.id} >
              <div><span>{item.pregnancy && (item.pregnancy.name + '——')}</span>{item ? item.note : ''}</div>
            </Menu.Item>
          );
        })}
      </Menu>
