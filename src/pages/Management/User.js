...
@connect(({ global, loading, userManagement }) => ({
...
class UserManagemant extends Component {
  constructor(props) {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          let { groups } = record;
          groups = groups.map(e => e.nickname);
          return <Ellipsis length={18}>{groups.join()}</Ellipsis>;
        },
      },
      {
        title: '状态',
        ...
        align: 'center',
        // width: 80,
        render: text => text ? <Button size="small" type="primary" ghost>激活</Button> : <Button size="small" type="danger" ghost>锁定</Button>,
      },
      {
        title: '创建日期',
      ...
      },
      {
        ...
        render: (text, record) => (
          <span>
            <Button icon="edit" size="small" type="primary" onClick={() => this.showModal('update', record)} />
  ...
  render() {
    ...
    const { visible, modalType, selectedUser } = this.state;
    return (
      <PageHeaderWrapper>
        <div className={styles.wrapper}>
          <div className={styles.left}>
            <Search placeholder="输入部门名称搜索..." onSearch={value => console.log(value)} enterButton />
...
const CustomizedForm = Form.create({
  ...
  return (
    <Form layout="inline" style={{ display: 'flex', alignItems: 'center' }}>
      ...
      <Button type="primary" icon="search" className="green-button" onClick={search}>搜索</Button>
      <Button type="primary" icon="plus" onClick={props.handleAdd}>新增</Button>
      <Button type="primary" icon="download" className="yellow-button" onClick={search}>导出</Button>
    </Form>
  );
});
  ...
  class extends React.Component {
    ...
    selectOptionChildren = (data = []) => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        children.push(<Option key={value.toString()} value={value.toString()}>{label}</Option>);
      }
      return children;
    };
          ...
          <Form {...formItemLayout}>
            ...
            <Form.Item label="角色选择">
              {getFieldDecorator('groups', {
                ...
                >
                  {this.selectOptionChildren(this.state.groups)}
                </Select>
                )}
            </Form.Item>
          </Form>
